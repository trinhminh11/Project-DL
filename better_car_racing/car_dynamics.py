...
class Car:
    ...
    def draw(self, surface, zoom, translation, angle, draw_particles=True, car_color = None):
        import pygame.draw



        if car_color is None:
            car_color = [127, 127, 127]
        else:
            car_color = car_color

        f_up = self.hull.fixtures[0]
        f_down = self.hull.fixtures[-1]

        trans = f_up.body.transform
        up_left, up_right= trans*f_up.shape.vertices[2], trans*f_up.shape.vertices[1]

        trans = f_down.body.transform
        down_left, down_right = trans * f_down.shape.vertices[3], trans * f_down.shape.vertices[0]

        ret_poly = [up_left, up_right, down_right, down_left]

        ret_poly = [(coords[0], coords[1]) for coords in ret_poly]
        
        ret_poly = [pygame.math.Vector2(c).rotate_rad(angle) for c in ret_poly]
        ret_poly = [
            (
                coords[0] * zoom + translation[0],
                coords[1] * zoom + translation[1],
            )
            for coords in ret_poly
        ]
        ...
        if draw_particles:
            ...
            pygame.draw.polygon(surface, color=car_color, points=ret_poly)
