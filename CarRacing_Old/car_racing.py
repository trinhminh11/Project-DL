...
class ModifiedCarRacing(gym.Env, EzPickle):
    """
    ## Description
    ...
    def __init__(
        ...
        self.vels = np.zeros(7, dtype=np.float32)

        self.observation_space = spaces.Dict({
            'image': spaces.Box(low=0, high=255, shape=(STATE_H, STATE_W, 3), dtype=np.uint8), 
            'rays': spaces.Box(low=0, high=self.ray_length, shape=(len(self.ray_angles), ), dtype=np.float32),
            'vels': spaces.Box(low=-np.inf, high=np.inf, shape=(7, ), dtype=np.float32)
        })
    ...
    def _render(self, mode: str):
        ...
        pygame.font.init()
        if self.screen is None and mode in ["human", "human_state"]:
            pygame.init()
            pygame.display.init()
        ...
        # showing stats
        self._render_indicators(WINDOW_W, WINDOW_H, mode)
    ...
    def _render_road(self, zoom, translation, angle, mode):
        bounds = PLAYFIELD
        field = [
            (bounds, bounds),
            (bounds, -bounds),
        ...
        self._draw_colored_polygon(
            self.surf, field, bg_color, zoom, translation, angle, clip=False
        )
        ...
        for poly in grass:
            ...
            self._draw_colored_polygon(
                self.surf, poly, grass_color, zoom, translation, angle
            )
        ...
        for poly, color in self.road_poly:
            ...
            tfm_poly = self._draw_colored_polygon(self.surf, poly, color, zoom, translation, angle)
    ...
    def _render_indicators(self, W, H, mode):
...
if __name__ == "__main__":
    ...
    env = ModifiedCarRacing(render_mode="human")
